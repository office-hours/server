...
public class GoogleTokenService implements ResourceServerTokenServices {
  ...
  public OAuth2Authentication loadAuthentication(String token)
    ...
    if (idToken != null) {
      ...
      User user = userService.findOrCreate(oauthKey, displayName, email);
      String role = (user.getTeacher() != null) ? "ROLE_TEACHER" : "ROLE_USER";
      Collection<GrantedAuthority> grants = Collections.singleton(
          new SimpleGrantedAuthority(String.format(ROLE_PATTERN, role)));
